...
var _ workout.Repository = &repository{}
...
func NewRepository(client postgresql.Client, logger *logging.Logger) workout.Repository {
	return &repository{
		client: client,
		logger: logger,
	}
}
func (r *repository) Create(ctx context.Context, wk workout.Workout) (string, error) {
...
func (r *repository) FindOne(ctx context.Context, id string) (*workout.Workout, error) {
	q := `
...
func (r *repository) FindAll(ctx context.Context) (w *[]workout.Workout, err error) {
	q := `
		SELECT * FROM workout;
	`
	r.logger.Trace(fmt.Sprintf("SQL Query: %s", utils.FormatQuery(q)))
...
func (r *repository) Update(ctx context.Context, id string, wk workout.Workout) error {
	tx, err := r.client.Begin(ctx)
	if err != nil {
		return err
	}
